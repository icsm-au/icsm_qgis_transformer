...
SOURCES = \
	__init__.py \
	icsm_qgis_transformer.py icsm_qgis_transformer_dialog.py

PLUGINNAME = icsm_ntv2_transformer

PY_FILES = \
	__init__.py \
	icsm_qgis_transformer.py icsm_qgis_transformer_dialog.py

UI_FILES = icsm_qgis_transformer_dialog_base.ui
...
deploy: compile transcompile
	...
	-rm $(HOME)/$(QGISDIR)/python/plugins/$(PLUGINNAME)/$(GRIDS)/*.gsb
...
zip: doc deploy dclean
	...
	rm -f $(PLUGINNAME).zip
	cd $(HOME)/$(QGISDIR)/python/plugins; zip -r $(CURDIR)/$(PLUGINNAME).zip $(PLUGINNAME) -x \help\images\*.png -x \help\*.md \*.gsb \*Makefile* \*.gsb \*.pyc \*.qrc
